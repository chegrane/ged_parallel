...
//            // optimal_Path.getG_cost();
    ...
    {//Path p = OPEN.poll();
if(Main.path_Stack.size()<15){ path_Stack1.push(p);}/// if(path_Stack1.size()>1) {Main.path_Stack.push(path_Stack1.get(0)); path_Stack1.remove(0);}}
    else{
    //  if(this.nb_all_path_added_to_open>0)
       {    path_Stack1.push(p); }  //      else
        //while(p.getG_cost_PLUS_h_cost() == OPEN.peek().getG_cost_PLUS_h_cost()) // Double comparison !!! To be fixed!
        //	OPEN.poll();
     }
    }

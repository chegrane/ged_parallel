...
//System.out.println("UBmain:"+Main.upper_bound);
        ...
        //while (!Main.path_Stack.empty()) // a loop to process all the vertices in g1, wa treat all the sub-tree.
        while (!Main.path_Stack.empty() && !is_time_out ) // a loop to process all the vertices in g1, wa treat all the sub-tree.
        {
                  ...
                  /*  if(optimal_Path==null)
                    ...
                    else*/
                    {
                        if (pMin.getG_cost() < Main.upper_bound.get())//optimal_Path.getG_cost())
                        {
                            optimal_Path = new Path(pMin); // Make a copy for debugging purpose
                            Main.upper_bound.set((int) pMin.getG_cost());
                            //System.out.println("ub:"+Main.upper_bound.get());
                        }
                    }
...
//            // optimal_Path.getG_cost();
    ...
    private void add_Path(Path p) {
        ...
        synchronized (Main.path_Stack) {
            ...
            if (Main.path_Stack.empty()) {
                Main.path_Stack.push(p);
            } else if (p.getG_cost_PLUS_h_cost() < Main.path_Stack.lastElement().getG_cost_PLUS_h_cost()) {
                Main.path_Stack.push(p);
            } else {
                Main.path_Stack.insertElementAt(p, Main.path_Stack.size() - 1);
            }
        }
    }

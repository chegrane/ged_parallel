...
//System.out.println("UBmain:"+Main.upper_bound);
        ...
        //  for(int i=0;i<size;i++)
            int se=0;
          while(se<size){synchronized(Main.path_Stack) {
                if(Main.path_Stack.size()>0){Path p1 = Main.path_Stack.pop(); path_Stack1.push(p1);
                se++;
          }
            }
            }
           // System.out.println("c bon size:"+size);
            while(!Main.path_Stack.empty()||!path_Stack1.empty()) // a loop to process all the vertices in g1, wa treat all the sub-tree.
            {
                ...
                if(start2-start1>60000)Main.path_Stack.clear();
            }
...
//            // optimal_Path.getG_cost();
        ...
        { Path p;
            //Path p = OPEN.poll();
            if(path_Stack1.size()>0){p = path_Stack1.pop();} else {  p = Main.path_Stack.pop();}
    ...
    {
        //Path p = OPEN.poll();
    if(Main.path_Stack.size()<5){ /*Main.path_Stack.push(p);}//*/if(path_Stack1.size()>1) {
        Main.path_Stack.push(path_Stack1.get(0)); path_Stack1.remove(0);
    }
    }

    // {path_Stack1.push(p);}
    // modifaction:
    if(p.getG_cost_PLUS_h_cost()<path_Stack1.lastElement().getG_cost_PLUS_h_cost())
    {
        path_Stack1.push(p);
    }
    else{

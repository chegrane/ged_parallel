...
// Exact Graph Edit Distance (GED) with Branch and Bound (BB) Depth First stratigy (DF)
    ...
    public double computeGED(Graph g1, Graph g2) {
        ...
        //System.out.println(" upper bound = "+Main.upper_bound);
        //System.out.println(" optimal path = "+optimal_Path);
        ...
        {
            //System.out.println("While Thrad = "+(Thread.currentThread().getName())+" iter = "+iter);
            //iter++;
            ...
            if (pMin != null) {
                if (pMin.isCompleteEditPath()) {
                 /*   if(optimal_Path==null)
                    ...
                    {
                        //System.out.println("if UB main:" +Main.upper_bound.get());
            ...
            if (start2 - start1 > (Main.amount_RunTime_S * 1000L)) {
                ...
                //System.out.println(" size Main Stack = "+(Main.path_Stack.size()));
        ...
        try {
            //System.out.println("await Thrad = "+(Thread.currentThread().getName()));
            barrier1.await();
...
//            // optimal_Path.getG_cost();
    ...
    private void add_Path(Path p) {
        //Path p = OPEN.poll();
        if (!path_Stack1.empty()){
            if (Main.path_Stack.size() < 5) { /*Main.path_Stack.push(p);}//*/
                if (path_Stack1.size() > 1) {
                    Main.path_Stack.push(path_Stack1.get(0));
